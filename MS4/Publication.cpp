...
namespace sdds {
	...
	std::ostream& Publication::write(std::ostream& os) const {
		if (conIO(os)) {
			// We must do it this way to be able to match the expected output
			char title[SDDS_TITLE_WIDTH + 1]{};
			strncpy(title, m_title, SDDS_TITLE_WIDTH);
			os << "| " << m_shelfId << " | ";
			os << std::setw(30) << std::left << std::setfill('.') << title << " | ";
	...
	std::istream& Publication::read(std::istream& istr) {
		int l_libRef = -1;
		int l_membership = 0;
		char l_title[SDDS_MAX_ARRAY_LEN]{};
		...
		if (conIO(istr)) {
			...
			std::cout << "Title: ";
			istr.getline(l_title, SDDS_MAX_ARRAY_LEN);
